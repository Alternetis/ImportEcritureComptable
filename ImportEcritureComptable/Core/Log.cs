...
#pragma warning restore CS0168 // La variable 'ex' est déclarée, mais jamais utilisée
        ...
        public static void WriteLog(string Value, bool NewLogGroup = false)
        {
            if (LogStream == null)
            {
                ...
                {
                    LogStream.WriteLine("");
                    LogStream.WriteLine("ImportEcritureComptable version : " + System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString());
                    LogStream.WriteLine("");
                    ...
                    //LogStream.WriteLine("Connexion base de données Sage : " + Properties.Settings.Default.ConnectionSage);
                    //LogStream.WriteLine("Connexion objets métiers Sage : " + Properties.Settings.Default.SageFile.Replace('\\','/').Split('/') + " - " + Properties.Settings.Default.SageUser.Replace("<", "&lt;").Replace(">", "&gt;"));
                    LogStream.WriteLine("");
                }
            }
            else if (NewLogGroup)
